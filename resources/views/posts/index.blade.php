            ...
            <div class="group block">
                <a href="/posts/{{ $post->id }}" class="block">
                    <div class="aspect-[2/3] overflow-hidden rounded-lg bg-gray-200 shadow-sm">
                        <img src="{{ asset('images/' . $post->cover) }}" alt="{{ $post->title }}" class="w-full h-full object-cover transition duration-300 group-hover:scale-105">
                    </div>
                    <div class="mt-2">
                        <h2 class="text-sm font-semibold text-gray-800 line-clamp-2 leading-snug transition group-hover:text-[#8B5CF6]">
                            {{ $post->title }}
                        </h2>
                    </div>
                </a>
                <p class="mt-1 text-xs text-gray-500 truncate">
                    <a href="{{ route('author.profile', $post->user->id) }}" class="text-xs text-gray-500 hover:text-[#8B5CF6] relative z-10">
                        {{ $post->user->name }}
                    </a>
                </p>
            </div>
